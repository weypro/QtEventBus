...
//    });
//    bus.subscribe<TestMessage>([](auto msg) { return QtPromise::resolve(msg.i + msg.j); });
//    bus.subscribe("test", QSubscriber(&receiver, &TestReceiver::test));
//    bus.subscribe(
//        "test", [](auto topic, auto message) { qDebug() << "test" << topic << message.toList(); });
//    bus.publish(TestMessage {1, 2})
//        .then(
//            [](QVector<int> const& result) { qDebug() << "TestMessage public result:" << result; },
//            [](std::exception& e) { qDebug() << e.what(); });
...
//    ;
    ...
    bus.subscribe<VoidMessage>([](auto) { return QtPromise::resolve(); });
    bus.subscribe("void", QSubscriber(&receiver, &TestReceiver::test));
//    bus.publish(VoidMessage {1, 2})
//        .then([]() { qDebug() << "VoidMessage public result"; },
//              [](std::exception& e) { qDebug() << e.what(); });
    bus.publish("void", QVariantList {1, 2})
        .then(
            [](QVector<QVariant> const& result) {
